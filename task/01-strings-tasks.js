...
function getRectangleString(width, height) {
    const drawLine = (start, fill, end) => `${start}${fill.repeat(width - 2)}${end}\n`;
    let rectangle = drawLine('┌', '─', '┐');
...
function encodeToRot13(str) {
    const rot13map = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'
        .split('')
        .reduce((acc, alphabetLetter, index) => {
            return {
                ...acc,
                [alphabetLetter]: 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm'[index]
            }
        }, {});
    return str.split('')
        .map(p => rot13map[p] || p)
